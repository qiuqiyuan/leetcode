    ...
    void recoverTree(TreeNode* root) {
        ...
        while(!s.empty() || node != NULL){
            if(node){
                ...
                s.pop();
                if(last){
                    if((node->val - last->val) < 0){
                    cout<< "DEBUG: " << last->val << " " << node->val <<endl;
                        if(fst == false && snd == false){
                            cout<< "DEBUG: push in last " << last->val <<endl; 
                            v.push_back(last);
                            fst = true;
                        }else if(fst == true && snd == false){
                            cout<< "DEBUG: push in node " << node->val <<endl; 
                            v.push_back(node);
                            snd = true;
                        }
                    }
                ...
                if(fst && snd) break;
                last = node;
                //cout<< "DEBUG: last->val: " << last->val <<endl;
                node = node->right;
            }
        }
        if(v.size() == 1) v.push_back(last);
